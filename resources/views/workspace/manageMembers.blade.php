...
@foreach ($workspace->users as $usr )
    @if($usr->pivot->ownership == 1 && $usr->id == $user_id || $usr->pivot->admin == 1 && $usr->id == $user_id)
        <form method="POST" enctype="multipart/form-data" action="/echrif.rayan/addusertoworkspace/{{ $workspace->id }}">
            @csrf
...
@foreach ($workspace->users as $usern)
    ...
    @if($usern->id != $user_id)
        ...
        @if($usern->id != $user_id && $usern->pivot->ownership == 0)
            <form method="POST" action="/echrif.rayan/changerole/{{ $workspace->id }}/{{ $usern->id }}">
                @csrf
            ...
            <form method="POST" action="/echrif.rayan/transferownership/{{ $workspace->id }}/{{ $usern->id }}"
            ...
            <form method="POST" action="/echrif.rayan/removeuserfromworkspace/{{ $workspace->id }}/{{ $usern->id }}"
